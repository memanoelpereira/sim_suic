...
# =========================================================
# PARÂMETROS DO MODELO — ATUALIZADOS PELO ÚLTIMO RELATÓRIO
# Fonte principal dos coeficientes pontuais:
#   path | frequentista | coeficientes
# Fonte principal dos intervalos de incerteza:
#   path | bayesiana | coeficientes
# =========================================================
# Estrutura:
...
# Coeficientes frequentistas centrais (último relatório)
BETA_XY = 0.30749     # percepcao_violencia -> ideacao_suicida
BETA_XM1 = -0.32133   # percepcao_violencia -> acolhimento
BETA_M1M2 = 0.44965   # acolhimento -> satisfacao_vida
BETA_M2Y = -0.16511   # satisfacao_vida -> ideacao_suicida

# Intervalos bayesianos aproximados (último relatório)
HDI_XY = (0.10834, 0.19787)
HDI_XM1 = (-0.35408, -0.26862)
HDI_M1M2 = (0.28175, 0.36987)
HDI_M2Y = (-0.16150, -0.07115)

# Efeitos condicionais frequentistas por nível de W (último relatório)
W_BAIXO = -1.74004
W_MEDIO = 0.0
W_ALTO = 1.74004
...
# Indireto simples via acolhimento
IND_M1_BAIXO = 0.10348
IND_M1_MEDIO = 0.11184
IND_M1_ALTO = 0.12020

# Indireto serial completo via acolhimento -> satisfacao
IND_SERIAL_BAIXO = 0.01575
IND_SERIAL_MEDIO = 0.02386
IND_SERIAL_ALTO = 0.03196

# Modulação nominal mantida para exibição comparativa
MOD_BAIXA = 0.80
MOD_NEUTRA = 1.00
MOD_ALTA = 1.20

# Médias populacionais da base atual
V_POP = 0.00
M1_POP = 0.00
M2_POP = 0.00
Y_POP = 0.00
...
def obter_efeitos_condicionais_por_nivel(nivel: str) -> tuple[float, float, float, float, float]:
    """
    Retorna:
    - valor de W
    - efeito direto
    - indireto via acolhimento
    - indireto serial completo
    - fator de modulação de exibição
    """
    if nivel == "baixa":
        return W_BAIXO, DIR_BAIXO, IND_M1_BAIXO, IND_SERIAL_BAIXO, MOD_BAIXA
    if nivel == "alta":
        return W_ALTO, DIR_ALTO, IND_M1_ALTO, IND_SERIAL_ALTO, MOD_ALTA
    return W_MEDIO, DIR_MEDIO, IND_M1_MEDIO, IND_SERIAL_MEDIO, MOD_NEUTRA
...
def simular_incerteza_risco(
    ...
    # Mantém os pontos centrais frequentistas, mas usa a incerteza bayesiana aproximada
...
def calcular_cadeia_serial_explicitada(
    ...
    """
    Explicita três momentos da cadeia serial com base nos efeitos condicionais
    do último relatório, em três cenários de pertença:
    1) risco direto de X
    2) risco após mediação do acolhimento
    3) risco após mediação conjunta de acolhimento e satisfação
    """
    valor_w, b_dir, ind_m1, ind_serial, mod = obter_efeitos_condicionais_por_nivel(nivel_pertenca)

    risco_direto = interceptos.ic_y + (b_dir * x)

    # X -> M1 previsto pelo coeficiente estrutural principal
    m1_previsto_por_x = interceptos.ic_m1 + (BETA_XM1 * x)

    # Etapa 2: adiciona apenas o indireto via acolhimento
    if usar_m1:
        risco_apos_acolhimento = risco_direto + (ind_m1 * x)
        m2_via_acolhimento = mod * max(m1_previsto_por_x, 0.0)
    else:
        risco_apos_acolhimento = risco_direto
        m2_via_acolhimento = 0.0

    # Etapa 3: adiciona a mediação serial completa
    if usar_m1 and usar_m2:
        risco_apos_cadeia_completa = risco_apos_acolhimento + (ind_serial * x)
        cadeia_ativa = True
    else:
        risco_apos_cadeia_completa = risco_apos_acolhimento
        cadeia_ativa = False
    ...
    return {
        ...
        "valor_w": valor_w,
        "mod": mod,
        "m1_previsto_por_x": float(np.clip(m1_previsto_por_x, 0.0, 5.0)),
        ...
        "cadeia_ativa": cadeia_ativa,
        "efeito_direto_condicional": b_dir,
        "efeito_indireto_acolhimento": ind_m1,
        "efeito_indireto_serial": ind_serial,
    }
...
def renderizar_diagrama_estrutural() -> go.Figure:
    ...
    anotacoes = [
        (0.24, 0.61, f"β X→M1 = {BETA_XM1:.3f}", "#d62728"),
        (0.52, 0.61, f"β M1→M2 = {BETA_M1M2:.3f}", "#2ca02c"),
        (0.78, 0.61, f"β M2→Y = {BETA_M2Y:.3f}", "#2ca02c"),
        (0.50, 0.73, f"β X→Y = {BETA_XY:.3f}", "#1f77b4"),
        (0.44, 0.30, "W: baixo / neutro / alto", "#ff7f0e"),
    ]
    ...
    fig.update_layout(
        ...
        plot_bgcolor="white",
        title=dict(text="Estrutura do Modelo Serial Moderado (coeficientes atualizados)", x=0.5, font=dict(size=13))
    )
    return fig
...
**Coeficientes centrais (path frequentista, último relatório)**
- X → Y = **{BETA_XY:.5f}**
- X → M1 = **{BETA_XM1:.5f}**
- M1 → M2 = **{BETA_M1M2:.5f}**
- M2 → Y = **{BETA_M2Y:.5f}**

**Intervalos bayesianos aproximados**
- X → Y: **[{HDI_XY[0]:.5f}, {HDI_XY[1]:.5f}]**
- X → M1: **[{HDI_XM1[0]:.5f}, {HDI_XM1[1]:.5f}]**
- M1 → M2: **[{HDI_M1M2[0]:.5f}, {HDI_M1M2[1]:.5f}]**
- M2 → Y: **[{HDI_M2Y[0]:.5f}, {HDI_M2Y[1]:.5f}]**

**Efeitos condicionais por pertença (path frequentista, último relatório)**
- Direto (baixo / médio / alto): **{DIR_BAIXO:.5f} / {DIR_MEDIO:.5f} / {DIR_ALTO:.5f}**
- Indireto via acolhimento: **{IND_M1_BAIXO:.5f} / {IND_M1_MEDIO:.5f} / {IND_M1_ALTO:.5f}**
- Indireto serial completo: **{IND_SERIAL_BAIXO:.5f} / {IND_SERIAL_MEDIO:.5f} / {IND_SERIAL_ALTO:.5f}**
...
- 2. Após acolhimento: risco após a mediação do acolhimento
- 3. Acolhimento + satisfação: risco após a mediação conjunta

**Importante**
- O app usa os **coeficientes pontuais frequentistas** do último relatório para as trajetórias principais e os **intervalos bayesianos** como aproximação da incerteza.
...
def exibir_bloco_cadeia_serial(x: float, inter: Interceptos, usar_m1: bool, usar_m2: bool, modo_pesquisador: bool):
    st.markdown("---")
    ...
    for col, (titulo, nivel) in zip(cols, cenarios):
        ...
        with col:
            ...
            if modo_pesquisador:
                st.caption(
                    f"W = {res['valor_w']:.2f} | mod = {res['mod']:.2f}"
                )
                st.caption(
                ...
                st.caption(
                    f"b_direto = {res['efeito_direto_condicional']:.5f} | "
                    f"ind_M1 = {res['efeito_indireto_acolhimento']:.5f} | "
                    f"ind_serial = {res['efeito_indireto_serial']:.5f}"
                )
            else:
    ...
    if modo_pesquisador:
        st.info(
            ...
            "O segundo acrescenta a mediação por acolhimento. "
            "O terceiro acrescenta a mediação conjunta de acolhimento e satisfação. "
            "O cenário neutro usa os efeitos médios do relatório; os cenários baixo e alto usam os efeitos condicionais por pertença."
        )
...
def main():
    ...
    st.caption("Modelo serial moderado: X → M1 → M2 → Y, com coeficientes atualizados pelo último relatório.")

    if "mem_x" not in st.session_state:
        st.session_state.mem_x = 1
        st.session_state.mem_m1 = 0
        st.session_state.mem_m2 = 0
    ...
    with col_esq:
        ...
        if modo_pesquisador:
            ...
            with b1:
                st.metric("X → M1", f"{BETA_XM1:.5f}")
                st.metric("M1 → M2", f"{BETA_M1M2:.5f}")
                st.metric("X → Y", f"{BETA_XY:.5f}")

            with b2:
                st.metric("M2 → Y", f"{BETA_M2Y:.5f}")
                st.metric("W baixo / neutro / alto", f"{W_BAIXO:.2f} / {W_MEDIO:.2f} / {W_ALTO:.2f}")
                st.metric("Ind. serial baixo / neutro / alto", f"{IND_SERIAL_BAIXO:.3f} / {IND_SERIAL_MEDIO:.3f} / {IND_SERIAL_ALTO:.3f}")
